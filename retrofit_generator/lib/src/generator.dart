        ...
        if (types.length > 1) {
          ...
          if (_typeChecker(List).isExactlyType(secondType) ||
            ...
            ''');
              ...
              case retrofit.Parser.FlutterCompute:
                log.warning('''
Return types should not be a map when running `Parser.FlutterCompute`, as spawning an isolate per entry is elanguagexmely intensive.
      ...
      if (const TypeChecker.fromRuntime(Map)
          ...
          case retrofit.Parser.DartJsonMapper:
            blocks.add(
              declareFinal(dataVar)
                  .assign(
                    refer('''
            JsonMapper.serialize(${bodyName.displayName})
            '''),
                  )
                  .statement,
            );
        ...
        if (clientAnnotation.parser == retrofit.Parser.MapSerializable) {
          ...
          if (_missingToJson(ele)) {
            ...
            blocks.add(
              declareFinal(dataVar)
                  .assign(refer('JsonMapper.serialize(${bodyName.displayName})'))
                  .statement,
            );
